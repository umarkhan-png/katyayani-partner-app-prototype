...
## 1. Status
**Enhance** — the inbox screen is designed; notification infrastructure barely exists.

## 2. Objective
Give the retailer one reliable place where every thing that happened to their money, orders, KYC, returns and requests is visible — and make each item a one-tap route to the screen where they can act.

## 3. Current State
- `notifications.html` is designed and is already the most complete Phase 2 screen: header with unread count ("4 unread updates"), **Mark all read**, filter tabs **All (10) / Orders / Offers / System**, day grouping (Today / Yesterday / Earlier), relative timestamps ("10 min ago", "Yesterday, 5:20 PM"), and an end-of-list marker "No more notifications".
- The designed items already span far more than orders — cart request from an advisor with a 22 h expiry, order shipped, price drop, target achieved, new scheme, credit payment due, community post, new course, KYC re-verification reminder, back-in-stock.
- In production, notifications are used **primarily for order placement** (push), with no inbox, no read state and no history.
- `partner_notifications` (Supabase) exists with exactly the right shape for an inbox — title, message, type, `link_url`, `is_read`, `created_at` — and **0 rows**. It has **no channel column, no delivery status, no provider message id, no read timestamp, no TTL and no broadcast/segment row**. Outbound push/WhatsApp/SMS lives in other systems.
- `app_announcements` (broadcast/targeted, type banner/notification/popup, segment/cohort/state targeting) also exists with **0 rows**.

## 4. Problem / Gap
1. **No event catalogue.** Features fire no events, so there is nothing for an inbox to display beyond order placement.
2. **Two half-systems.** Per-partner rows (`partner_notifications`) and broadcast rows (`app_announcements`) both exist and neither is used. The inbox must read **one merged feed**, and which table owns what must be decided before any code is written.
3. **No fan-out model.** A scheme announcement to 20,000 retailers is 20,000 rows in the current design — no audience row, no cohort inheritance.
4. **No deep-link registry.** `link_url` is unvalidated free text. Without a registry, a typo becomes a dead tap in production.
5. **No read analytics.** `is_read` flips without a timestamp, so time-to-read and notification effectiveness are unmeasurable.
6. **No retention.** Nothing prunes the feed; it grows unbounded.

## 5. Proposed Solution

**Three layers, built in this order:**

1. **Event layer (MH).** Every Phase 2 feature emits domain events (listed per feature in this document). One shared envelope: `event_name`, `retailer_id`, `entity_type`, `entity_id`, `payload`, `occurred_at`.
2. **Notification service (MH).** Maps event → notification template (title, body, category, deep link, priority, channel set) → writes the inbox row and, where the template says so, dispatches push. Templates are **localised** — the app already supports regional languages, and `retailers_v2.language` is the language of record.
3. **Inbox (MH).** The designed screen, reading the merged feed.

**Categories (Phase 2 final list — only events that real Phase 2 or live features can actually produce):**

| Category | Events |
| --- | --- |
| **Orders** | placed · confirmed · processing · packed · shipped · out for delivery · delivered · cancelled · failed · delayed · payment status change |
| **Returns** | request created · approved · rejected · pickup scheduled/updated · replacement update · refund initiated · refund completed |
| **KYC** | submitted · approved · rejected · additional document required · Supporting ID verification update · document expiring |
| **Money** | wallet credited · wallet debited · coins earned · coins redeemed · coins expiring · cash deposit update · payment failure |
| **Business** | new scheme · deal/coupon · VIP tier update · leaderboard update · new training content · product back in stock / price change · referral update · sales cart request · inventory low-stock + add-delivered-stock prompt |

**Inbox tab mapping** — keep the four designed tabs and map categories to them so the UI does not change: **All** · **Orders** (orders + returns) · **Offers** (business/marketing) · **System** (KYC + money + account). Adding a fifth tab is **NH**.

**Priority model (MH):** `action_required` (cart request, KYC rejected, payment failed, return needs info) sorts above `informational` within the same day group and is the only class allowed to also fire push at low-engagement hours. Quiet hours — **Open Decision**.

## 6. User Flow
```
Home → Bell icon (with unread badge) → Notification inbox
→ [tab filter] → Tap notification → Mark read → Deep link to target screen → Act
Alternate: Push notification (app closed) → Deep link opens target screen directly
           → Bell badge decrements → item already marked read in inbox
Bulk: Mark all read → all unread flip → badge clears
```

## 7. Screen-by-Screen UX

**S1 · Home bell entry (existing header — badge only)**
- *Purpose:* signal that something needs attention.
- *Key UI:* bell icon with numeric badge, capped display at "9+".
- *Rule:* badge counts **unread**, not "new since last open".

**S2 · Notification inbox** (`notifications.html`, as designed)
- *Purpose:* one chronological, filterable feed.
- *Key UI:* title + unread count, Mark all read, tabs with counts, day-grouped rows. Each row: category icon, title (1 line), body (max 2 lines), relative time, unread dot, and an optional inline CTA for `action_required` items (e.g. "Review & accept" on a cart request, with its expiry chip "Expires in 22h").
- *Primary CTA:* tap row → deep link. *Secondary:* Mark all read; swipe-to-read is **NH**.
- *Navigation:* back to Home; forward to the deep-link target.
- *States:* Loading (skeleton rows), Default, Empty per tab, Error, End of list ("No more notifications" — already designed), Paginating.

**S3 · Deep-link targets** — no new screens. Every notification resolves to an existing screen: order details, return details, KYC hub, wallet/coins, scheme/PDP, cart request, inventory, leaderboard, training.

## 8. States
Default · Loading (first load + pagination) · Empty (no notifications ever / empty tab) · Error (fetch failed → Retry) · Success (mark-read applied) · Expired (item whose target expired, e.g. a lapsed cart request — shown greyed with "Expired", still tappable to a read-only view) · Disabled (Mark all read when nothing unread) · Offline (cached feed with an offline strip)

## 9. Business Rules
1. Tapping a notification marks it read. Opening the inbox does **not** mark everything read.
2. Deep links must resolve to a valid screen or fall back to the relevant list screen — **never a blank screen or a crash**.
3. Retention / auto-archive window — **Open Decision — Business confirmation required** (today nothing expires).
4. Which categories may also fire push vs inbox-only — **Open Decision — Business confirmation required**.
5. Quiet hours and per-retailer frequency caps — **Open Decision**.
6. Notification language follows `retailers_v2.language`; if unset, app language, then Hindi. **Open Decision on the default.**
7. Notification preferences (per-category opt-out) — **NH for Phase 2**, flagged because Settings already exists.

## 10. Backend / System Requirements
- **Event bus / outbox** — a reliable path from feature events to the notification service. Does not exist today.
- **Notification service + template store** — template per event, per language, with the deep link and channel set.
- **Inbox API** — paginated feed (cursor), unread count, mark-read, mark-all-read.
- **Schema gaps to close on `partner_notifications` (or its replacement):** `category`, `read_at`, `priority`, `entity_type` + `entity_id`, `expires_at`, and an audience model for broadcasts. As it stands the table cannot support the designed screen's tabs, expiry chips or analytics.
- **Push delivery** — the existing push stack (whatever sends order-placed today) must be identified and reused, not rebuilt. **Open Decision.**
- **Deep-link registry** — a single enumerated list of routes shared by app, backend and templates.
- Consumes events from: Orders · Returns (F4) · KYC (F1) · Wallet/Coins (Phase 1) · Referral (F5) · Sales cart request (F7) · Inventory (F11) · Schemes/VIP/Leaderboard/Training (Phase 1).

## 11. Notifications
This feature *is* the notification surface; it emits only one of its own: **push-permission re-prompt** (**NH**) when a retailer has push disabled and an `action_required` item is pending.

## 12. Analytics
`notification_inbox_opened` (unread_count) · `notification_tab_switched` (tab) · `notification_viewed` (category, event_name, position) · `notification_tapped` (category, event_name, age_minutes) · `notification_deeplink_resolved` / `_failed` (route) · `notification_marked_read` · `notification_mark_all_read` (count) · `notification_push_received` / `_opened` / `_dismissed` · `notification_inbox_empty_viewed` (tab) · `notification_pagination_loaded` (page)

## 13. Edge Cases
- Push tapped while the app is cold-started → deep link must survive the launch, not drop to Home.
- Notification arrives for an entity that was deleted or cancelled meanwhile.
- Cart-request notification tapped after its 22 h expiry.
- Same event fired twice (retry) → inbox must de-duplicate on `(retailer_id, event_name, entity_id)`.
- Retailer has push disabled at OS level — the inbox must still receive everything.
- Language changed after a notification was written — the stored row keeps the old language (acceptable; flag in QA).
- 500+ unread items — pagination and badge cap behaviour.
- Clock skew between server and device making an item read "in 2 minutes".
- Two devices, same account, both marking read.
- Broadcast to a segment the retailer has since left.

## 14. Dependencies
**Partner App** (every feature that emits) · **RLM Admin / Sales Operation Portal** (who authors broadcast notifications and schemes) · **Support** (ticket/status updates) · **Accounts** (wallet, refund, payment events) · **Dispatch** (shipment milestones) · **Push provider** (existing, to be identified)

## 15. Acceptance Criteria
1. Every Phase 2 feature listed in §10 produces at least one inbox item end-to-end in staging.
2. The unread badge equals the count of unread items and clears correctly after Mark all read.
3. Tapping any item opens the correct screen for that entity; no route in the registry is unhandled (QA runs the full route list).
4. Tapping an item marks it read within one refresh, on both devices for a two-device account.
5. Each tab filters correctly and shows its own empty state with the right copy.
6. Feed paginates without duplicates or gaps at 200+ items.
7. A push tapped from a cold start lands on the target screen, not Home.
8. Duplicate events do not create duplicate rows.
9. An expired `action_required` item renders as Expired and opens a read-only view rather than a broken action.
10. Notification copy renders in the retailer's language for every template in the catalogue.
11. Offline: the last-loaded feed is visible with an offline indicator, and mark-read syncs on reconnect.

## 16. Out of Scope
In-app chat · WhatsApp/SMS campaign management · per-category notification preferences (**NH**) · rich media in notifications · notification scheduling UI for marketing (belongs to Admin, not the app).

## 17. Open Decisions
1. Which table is the inbox store — `partner_notifications` (needs columns added), a new table, or the CRM Mongo stack the live app already uses?
2. How are broadcasts modelled — fan-out rows, or an audience row the app resolves at read time?
3. Which push provider is already sending order-placed notifications today?
4. Retention / auto-archive window.
5. Which categories are push-eligible; quiet hours; frequency caps.
6. Who authors and approves marketing/scheme notifications operationally?
7. Is the full deep-link route list owned by the app team or defined jointly with backend?
...
| F6 | `testimonial.html`, `testimonial-success.html` (story-level), review panels in `order-details.html` (product-level) | Re-anchor to product-level |
| F7 | `sales-cart-request.html`, item in `notifications.html` | RLM composer does not exist |
| F8 | — | No screen exists |
| F9 | `ai-chatbot.html`, `scanner.html` (sibling) | Scripted demo |
| F10 | `my-farmers.html` | No profile screen yet |
| F11 | `inventory.html` | Richest prototype; needs the delivered-order mechanic |
| F12 | `home.html` (story rail + viewer) | Testing only |
