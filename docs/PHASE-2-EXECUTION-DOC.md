...
Phase 1 is **live** (regional languages, Supporting IDs + AI document scan, KYC flow, new home, Shop by Crop, catalog filters, enhanced PDP, Wallet + Coins, Statement, Cash Deposit, Profit Calculator, Leaderboard, VIP Tier, Shop by Disease, Quick Order, Training Videos, Business Dashboard, Poster Generator). Phase 1 screens are referenced only where a Phase 2 feature extends them.

| Tag | Meaning |
| --- | --- |
| **MH** | Must Have — blocks release |
| **NH** | Nice to Have — cut first |
| **Open Decision** | Not decided. Needs Product / Business / Tech confirmation. Nothing invented here. |
| **Provider-dependent** | Depends on an external provider / SDK / OS capability, not on our code |

**Designed ≠ built ≠ wired.** A prototype screen is UI only — no API, no state machine, no events. Each feature below separates the three.

### Phase 2 scope

| # | Feature | Status | Designed | Built | Wired | The real work |
| --- | --- | --- | --- | --- | --- | --- |
| 1 | PIN-based shop address auto-fill | Enhance | Yes — `location-pin.html`, `shop-address-details.html` | No | No | Maps/Places/Geocoding contract, plus-code fallback, `pincode_map_v2` reconciliation |
...
| --- | --- | --- |
| `pincode_map_v2` | CRM Mongo | **Live.** 19,300 pincodes → district / state / territory / taluk / `geo{lat,lng}`, source India Post. `pincode` is **not indexed**; taluk missing on 418 and `"NA"` on 1,047; `geo.lat` null on 1,096. |
| `orders_v2` | CRM Mongo | **Live.** `order_status` + `order_status_map[]`, `booking_details.courier_status`, AWB. Delivery timestamp for the inventory prompt comes from here. |
| `products_v2` | CRM Mongo | **Live.** Catalog of record — pack and case data for the inventory unit conversion. |
| `retailers_v2` | CRM Mongo | **Live.** `is_verified`, `language` (notification language), `vip_level`. |
| `partner_notifications` | Sales-CRM (Supabase) | **Built, 0 rows.** Missing category, `read_at`, priority, entity ref, expiry, audience model. |
...
# FEATURE 1 — PIN-Based Shop Address Auto-Fill (Map-First)
...
| Fields | Shop name\* · shop no./floor/building\* · landmark | Type · name\* (Other only) · no./floor/building\* · landmark · default toggle |
...
- **Three ways to get the pin right**, all equal citizens: (a) auto-detect, (b) **search** (Google Places Autocomplete — area, street, mandi, landmark), (c) **drag the map** under a fixed centre pin. With location switched off, the sheet offers **Enter address manually** — not another search prompt.
...
### When the map has no street address (MH)

Rural and semi-urban pins often come back as a **Plus Code** (`23XY+TF Sanwer`) or with no `route` component at all. **This gets no special screen.** The rule is simply:

1. **A plus code is never shown or stored.** It is dropped before the fields are filled.
2. Whatever the geocoder did return — locality, city, pincode — lands in the normal fields.
3. **Area / City / Tehsil is an editable field anyway**, so a retailer whose pin resolved poorly types it like any other correction. No fallback state, no different copy, no branch in the flow.
4. The pin's lat/long is saved regardless — the coordinates are the accurate part, and they are what the delivery partner navigates to.
...
- **Name first, mandatory.** `Shop name *` on the shop journey; on the address journey only Other asks one (`Specify other *`). **Shop, Warehouse and Home ask for no name** — the shop name is already on the account, and the type tile and the premises line already identify the place, and asking again only adds a field.
...
- **Area / City / Tehsil** is one editable field, pre-filled from the map and badged *From map*; editing it flips the badge to *Edited by you*.
- **Pincode is editable and is the only thing that changes District and State** — both are read-only and derive from `pincode_map_v2` the moment six digits are entered. A retailer can correct a wrong pin-derived pincode without touching the map.
- **Change location** sits on the map strip and returns to the pin.
...
| --- | --- | --- |
| **Shop** | *not asked* | Shop no. / Floor / Building name * |
| **Warehouse** | *not asked* | Gate no. / Godown / Building name * |
...
2a. **Area / City / Tehsil and Pincode are editable; District and State are not** — they follow the pincode. A wrong locality can be fixed by typing or by moving the pin; a wrong district cannot be typed at all. **MH.**
...
## 12. Analytics
`location_screen_viewed` (context: onboarding | add_address) · `location_permission_prompted` / `_granted` / `_denied` · `location_autodetect_succeeded` (accuracy_m, time_ms) / `_failed` · `location_search_opened` · `location_search_query` (length) · `location_search_result_selected` (rank) · `location_search_no_result` (query) · `map_pin_dragged` (distance_m, count) · `geocode_returned_pluscode` — **keep it even though the UI no longer branches: it sizes how often retailers must type the area themselves** · `location_confirmed` (source: gps | search | drag, completeness) · `address_form_viewed` · `address_type_selected` (type) · `address_area_edited` · `address_pincode_edited` (changed_district: true/false) · `address_saved` (fields_typed_count, completeness) · `address_save_failed` (reason) · funnel: map opened → pin confirmed → details saved
...
5. **A plus code never appears anywhere in the UI or in the saved record**, and produces no separate state — the retailer simply edits Area / City / Tehsil like any other correction.
...
9. Saving is blocked with an inline message, and the field scrolled into view, when: name is empty (except Home) · premises is empty · Area / City / Tehsil is empty · pincode is not six digits.
10. Editing the pincode refills District and State; an unknown pincode leaves them blank and flags the address for a check rather than blocking the save.
...
> Returns are **not in Phase 2**. These templates are written and wait for the return feature — building them now would send retailers to a screen that does not exist.
...
> KYC is **live from Phase 1**, so these ship with Phase 2.
...
> Rows marked **later** depend on a feature outside Phase 2 (referral, reviews). Everything else ships in Phase 2 — cart request and inventory are Phase 2 features, the rest are live Phase 1 surfaces.
...
| B6 | `referral.joined` **(later)** | _Vikas_ joined | _Vikas Kheti Mart_ signed up with your code. _₹500_ comes after their first delivered order of _₹2,000+_. | Refer & Earn | Yes | Info |
| B7 | `referral.step_done` **(later)** | _Vikas_ completed KYC | _Vikas Kheti Mart_ is verified. One step left — their first order of _₹2,000+_. | Refer & Earn | No | Info |
| B8 | `referral.rewarded` **(later)** | _₹500_ referral reward | _₹500_ credited for referring _Ramesh Agro Centre_. | Wallet | Yes | Info |
| B9 | `referral.rejected` **(later)** | Referral not eligible | _Sharma Krishi Kendra_ was already a Katyayani retailer, so this referral does not qualify. | Refer & Earn | No | Info |
| B10 | `review.requested` **(later)** | Rate what you received | How did _Katyayani Bhumiraja_ work? Your review helps other retailers. | Rate products | Yes | Info |
| B11 | `review.published` **(later)** | Your review is live | Your review of _Katyayani Bhumiraja_ is now on the product page. | PDP | No | Info |
| B12 | `review.rejected` **(later)** | Review not published | Your review of _Chakraveer_ was not published — _competitor name mentioned_. You can edit and resubmit. | Rate products | No | Info |
...
# FEATURE 3 — Sales Cart Request (B2B Bulk via Phone)

> **The portal side has its own document:** `docs/RLM-CART-REQUEST.md` — how an RLM agent composes and sends the request inside **Katyayani RetailHub**, the approval path, the request board and the data mapping (a cart request is a `partner_quotations` row; accepting writes `partner_cart`).
...
# FEATURE 4 — Inventory Tracker (Per SKU)
...
What the retailer shares on WhatsApp is their own bill to the farmer, so it is laid out as a real document, not a receipt. The retailer picks the type in the sell flow, **before** the bill is generated:

| Choice | What changes |
| --- | --- |
| **Tax invoice** (default) | Marked *Tax Invoice*, numbered `INV-…`, seller GSTIN printed, each line shows its GST rate, and the totals break into taxable value + CGST + SGST |
| **Bill of supply** | Marked *Bill of Supply*, numbered `BOS-…`, **no GSTIN line, no tax rows** — just Subtotal and Total — the line reads “No GST charged”, and the footer states plainly: *Not a tax invoice — no GST has been charged on this bill* |

Both share the same structure:

| Block | Contents |
| --- | --- |
| Seller | Shop name · full shop address · **GSTIN** · **pesticide licence number** · phone |
| Document | "Tax Invoice" mark · invoice number · date |
| Bill to | Farmer name · village · phone |
| Items | Product (with its GST rate) · qty · rate · amount — rate is the shelf price, **inclusive of GST** |
| Totals | Taxable value · CGST · SGST · **Total** · amount in words |
| Payment | Received in full, or Due / Udhaar with the pending amount |
| Footer | "For {shop}" signature line · Insecticides Act 1968 note · computer-generated-invoice line |

**Open Decisions:** whether the type should be **picked per sale at all**, or fixed by the retailer’s own registration (a composition dealer may never issue a tax invoice, and an unregistered retailer never may) · a composition dealer’s bill must carry the statutory declaration, which this design does not print yet · where GST rates come from (product master per SKU — pesticides and fertilisers differ) · whether a retailer who is **not GST-registered** should see tax lines at all, or a plain bill of supply · whether invoice numbering must be a gapless series per financial year (it must be, if these are treated as statutory invoices) · HSN codes on each line.
...
[Sell path]  Inventory → Sell to Farmer → Customer → Product/Qty/Price/Payment → Invoice
             → Stock decreases + ledger entry + the sale saved against that customer
...
- *Addition:* stock is decremented, a ledger entry is written, and the sale is saved against the customer record inside Inventory.
...
- **Sales records** — stored with the sale (farmer name, products, amount, payment status, invoice) — My Farmers is out of Phase 2, so this list lives inside Inventory for now.
...
## 14. Dependencies
**Partner App** Orders (delivered lines) · Cart/Reorder · `products_v2` catalog · **Sell-to-Farmer customer records** (kept inside Inventory — My Farmers is not in Phase 2) · **Notifications inbox (Feature 2)** · **Business Dashboard** (Phase 1) if inventory metrics feed it
...
Build once, before the features that consume them.

| Component | Used by | Must do |
| --- | --- | --- |
...
```
            ┌──────────────────────────────────────────────┐
            │  EVENT BUS + NOTIFICATION SERVICE (Feature 2) │
            └──────────────────────────────────────────────┘
                 ▲                ▲                  ▲
                 │ emits          │ emits            │ emits
        ┌────────┴───────┐  ┌─────┴────────┐  ┌──────┴───────┐
        │ Orders (live)  │  │ Cart request │  │ Inventory    │
        │ KYC, Wallet,   │  │ (Feature 3)  │  │ (Feature 4)  │
        │ Coins (live)   │  └─────┬────────┘  └──────┬───────┘
        └────────────────┘        │                  │
                                  ▼                  ▼
                         Cart / Checkout      Orders: delivered
                              (live)          lines + timestamp
        ┌──────────────────────────────┐
        │ Address (Feature 1)          │  standalone — shares only
        │ pincode_map_v2 + Maps        │  the permission component
        └──────────────────────────────┘
```

| Feature | Depends on | Nature |
| --- | --- | --- |
| **1 · Address** | `pincode_map_v2` (+ index), Google Maps/Places/Geocoding account | Hard on the maps account, otherwise standalone |
...
1. **Store of record** — is the Partner App on the Supabase `partner_*` tables or the CRM Mongo/RLM stack? (Blocks 2 and 3.)
2. **Cart contract** — is the live cart quotation-scoped (`partner_cart`) or free-catalog? (Blocks 3.)
3. **Google Maps Platform account** — billing owner, key restrictions, session tokens. (Blocks 1.)
4. **Delivered-order data contract** — which lines were delivered, when. (Blocks 4.)
5. **Event bus + notification schema** — the spine for 2, and the destination for 3 and 4 events.
6. **Shared components** (§A) and the **deep-link registry**.

### Wave 1 — Address (Feature 1)
Smallest dependency surface, both screens designed, `pincode_map_v2` already live. Needs the maps account and the `pincode` index. Improves every delivery that follows.

### Wave 2 — Notifications inbox (Feature 2)
Ship the inbox against **live Phase 1 events first** (orders, KYC, wallet, coins, schemes), so it has real content on day one. Features 3 and 4 then plug their events into a working pipe.

### Wave 3 — Inventory tracker (Feature 4)
Ledger first, then the delivered-order suggestion (the adoption mechanic), then low-stock → reorder. Emits `inventory.delivered_stock` and `inventory.low_stock` into the inbox from Wave 2.
...
| --- | --- | --- | --- | --- |
| R1 | **Store-of-record ambiguity** — `partner_*` tables built but empty; the live app may be elsewhere | 2, 3 | Estimates unreliable; risk of building against the wrong database | Resolve in Wave 0, in writing, before any backend ticket |
| R2 | **Cart contract conflict** — `partner_cart` is quotation-scoped, a free-catalog request cannot use it | 3 | Rework, or a second cart system | Settle in Wave 0 |
| R3 | **RLM composer unresourced** — it lives outside the app | 3 | App screen ships with nothing behind it | Name an owner at the start; defer the feature if unfunded |
| R4 | **Maps cost and plus codes** — Autocomplete + Geocoding at onboarding scale; rural pins return plus codes | 1 | Bill surprise; meaningless addresses on labels | Session tokens, key restrictions, cost ceiling; plus-code rule (drop the code, keep pincode/district/state + pin) |
| R5 | **Notification architecture gaps** — no audience model, no `read_at`, no TTL | 2 | Fan-out cost, unmeasurable effectiveness, unbounded growth | Decide broadcast model and add columns before go-live |
| R6 | **Inventory unit conversion** — case ⇄ unit errors | 4 | One wrong auto-add destroys trust in the feature permanently | Confirmation before add (never silent), conversion line shown, editable quantities, full ledger |
...
### Blocking — before development starts
...
| 5 | Who builds and funds the **RLM cart composer** | Operations + Tech | 3 |
| 6 | Notification store + broadcast model + the missing columns | Tech + Product | 2 |

### Business

| # | Decision | Feature |
| --- | --- | --- |
| 7 | Do non-serviceable / blacklisted pincodes block onboarding, warn, or pass? | 1 |
| 8 | May a retailer override auto-filled state/district? Does a post-KYC address change need re-verification? | 1 |
| 9 | Notification retention window, push-eligible categories, quiet hours, frequency caps | 2 |
| 10 | Who authors and approves scheme / marketing notifications | 2 |
| 11 | Cart-request expiry duration; price honoured after expiry?; duplicate-SKU merge policy (sum vs replace) | 3 |
| 12 | Advisor discount / "advisor bonus" authority and approval limits | 3 |
| 13 | Can a request go to a retailer whose KYC is pending? Credit-limit and MOV checks at accept or at checkout? | 3 |
...
| # | Decision | Feature |
| --- | --- | --- |
| 17 | Reverse geocoding — paid provider or nearest-neighbour over `pincode_map_v2.geo` | 1 |
| 18 | Who approves the `pincode` index on `pincode_map_v2` (production DB change) | 1 |
| 19 | Is `addresses` (CRM Mongo) the store of record, and who approves adding lat/long, accuracy, source, place_id | 1 |
| 20 | Which push provider sends order-placed today | 2 |
| 21 | Territory rules — which agent may raise a request for which retailer | 3 |
...
## Appendix A — Prototype screens for Phase 2
...
Specs live in **`docs/PHASE-2-DEFERRED-FEATURES.md`**; prototypes stay in the repo and still work.
...
| Product catalog download | `catalog-download.html` |
| Krishi AI chatbot | `ai-chatbot.html`, `scanner.html` |
| My Farmers (CRM-lite) | `my-farmers.html`, `farmer-profile.html` |
| Instagram-style story rail | `home.html` (rail + viewer, `?story=`) |
| Screen share with support | `screen-share.html` |

---

*Prepared from the Partner App prototype (`katyayani-partner-app/screens/`), the KKD prototype, the development plan CSV, and the Katyayani Knowledge Base schema vault (CRM Mongo + Sales-CRM Supabase). Every backend statement was checked against those sources; anything not found there is marked as an Open Decision rather than assumed.*
