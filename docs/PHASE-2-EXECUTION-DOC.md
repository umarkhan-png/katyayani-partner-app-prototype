...
- **Name first, mandatory.** `Shop name *` on the shop journey; on the address journey the label follows the type (`Specify other *`). **Warehouse and Home ask for no name** — the type tile and the premises line already identify the place, and asking again only adds a field.
